...
// >> BLE
TaskHandle_t Task1;
int globalSeenNotifiers = 0;
// https://github.com/kmetz/BLEExposureNotificationBeeper/blob/master/src/main.cpp
#include <BLEDevice.h>
#include <BLEScan.h>
...
// When to forget old senders (identity change every 10-20 minutes)
#define FORGET_AFTER_MINUTES 10
...
void onNewNotifierFound() {
  Serial.println("New BLE Notifier found");
  globalSeenNotifiers = seenNotifiers.size();
}
...
void setup()   {
  ...
  xTaskCreatePinnedToCore(
                    ...
                    (void*)&globalSeenNotifiers,  /* parameter of the task */
...
void sendDustAndNo() {
  if (wifi_connected) {
   if (!client.connected()) {
    ...
    rssi = WiFi.RSSI();
    snprintf (msg, 250,"{\"rssi\": \"%i\",\"r1\": \"%i\",\"corona_apps\": \"%i\" }", rssi, a_no, globalSeenNotifiers);
    Serial.println(msg);
    Serial.println(data_topic);
    client.publish(data_topic.c_str(), msg); 
    a_no = 0;
